...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const vendorName = params.slug.charAt(0).toUpperCase() + params.slug.slice(1);
  
  return {
    title: `${vendorName} - Yip In Tsoi`,
    description: `Explore ${vendorName} solutions and partnerships`
  };
}

export default async function VendorPage({ params }: PageProps) {
  const vendorSlug = params.slug.toLowerCase();
  
  // Simple vendor data based on slug
  ...
  const vendor = vendorData[vendorSlug];
                  ...
                  <ScrollReveal key={offering.id} delay={index * 0.05}>
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {offering.tags.slice(0, 3).map((tag: string) => (
                          <span
                            key={tag}
